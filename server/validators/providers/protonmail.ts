...
export async function validateProtonmail(email: string, host: string): Promise<ValidationResult> {
  return new Promise((resolve) => {
    ...
    const commands = [
      'EHLO protonmail-validator.local\r\n',
      `MAIL FROM:<verify@protonmail-validator.local>\r\n`,
      `RCPT TO:<${email}>\r\n`,
      'QUIT\r\n'
    ];
    ...
    socket.on('data', (data) => {
      ...
      switch (stage) {
        ...
        case 3:
          if (response.code === 250) {
            closeWithResult(true, 'ProtonMail address verified');
          } else if ([550, 551, 553].includes(response.code)) {
            closeWithResult(false, 'ProtonMail address does not exist');
          } else {
            closeWithResult(false, 'ProtonMail verification failed');
          }
          break;
      }
    });
