...
export async function validateGmail(email: string, host: string): Promise<ValidationResult> {
  return new Promise((resolve) => {
    ...
    const commands = [
      'EHLO gmail-validator.local\r\n',
      `MAIL FROM:<verify@gmail-validator.local>\r\n`,
      `RCPT TO:<${email}>\r\n`,
      'QUIT\r\n'
    ];
    ...
    socket.on('data', (data) => {
      ...
      switch (stage) {
        ...
        case 3:
          if (response.code === 250) {
            closeWithResult(true, 'Gmail address verified');
          } else if ([550, 551, 553].includes(response.code)) {
            closeWithResult(false, 'Gmail address does not exist');
          } else {
            closeWithResult(false, 'Gmail verification failed');
          }
          break;
      }
    });
