...
export async function validateYahoo(email: string, host: string): Promise<ValidationResult> {
  return new Promise((resolve) => {
    ...
    const commands = [
      'EHLO yahoo-validator.local\r\n',
      `MAIL FROM:<verify@yahoo-validator.local>\r\n`,
      `RCPT TO:<${email}>\r\n`,
      'QUIT\r\n'
    ];
    ...
    socket.on('data', (data) => {
      ...
      switch (stage) {
        ...
        case 3:
          if (response.code === 250) {
            closeWithResult(true, 'Yahoo address verified');
          } else if ([550, 551, 553].includes(response.code)) {
            closeWithResult(false, 'Yahoo address does not exist');
          } else {
            closeWithResult(false, 'Yahoo verification failed');
          }
          break;
      }
    });
